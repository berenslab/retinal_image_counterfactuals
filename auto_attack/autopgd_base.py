...
class APGDAttack():
    ...
    def __init__(
            ...
            eps_increase_steps=None,
            eps_increase_factor=None
    ):
        """
        AutoPGD implementation in PyTorch
        """

        self.eps_increase_steps = eps_increase_steps
        self.eps_increase_factor = eps_increase_factor
        self.masks = masks
    ...
    def attack_single_run(self, x, y, x_init=None, first_run=False, ODI_annealing_init=False):
        ...
        if x_init is not None:
            ...
            if self.norm == 'Linf':
                ...
                w_randn = torch.randn(x.shape).to(self.device).detach()
                x_adv = self.get_lmo_update(x, x, w_randn, 1)
                #print('same noise for all channels')
                #x_adv = x_adv[:, 0, :, :].unsqueeze(1).tile((1, 3, 1, 1))
                print('x_adv_1 nan in tensor', x_adv.isnan().any())
                print(f'{get_norm(self.norm)}-norm is', (x_adv-x).view(x.shape[0], -1).norm(dim=1, p=get_norm(self.norm)))
        ...
        if self.eps_increase_steps is not None:
            print('creating video arr')
            arr_video = []
        for i in tqdm(range(self.n_iter)):
            ### gradient step
            if self.eps_increase_steps is not None:
                if (i > 0) and (i % self.eps_increase_steps == 0):
                    self.eps += self.eps_increase_factor
                    print('new eps', i, self.eps)
            ...
            if self.eps_increase_steps is not None:
                if i == self.n_iter - 1:
                    assert ((i+1) % self.eps_increase_steps == 0), f'Iter {i} should be divisible by {self.eps_increase_steps}'
                if (i > 0) and ((i+1) % self.eps_increase_steps == 0):
                    arr_video.append(x_best.detach().clone().cpu())
                    print('appending to video arr', len(arr_video))
                    if len(arr_video) > 1:
                        print('video arr norms', (arr_video[-1]-arr_video[-2]).view(len(arr_video[-1]), -1).norm(p=2))
        #

        if self.return_all_losses_confid:
            return (x_best, acc, loss_best, x_best_adv, arr_video if self.eps_increase_steps is not None else None, torch.cat(all_losses, dim=1), torch.cat(all_confid, dim=2))
        else:
            return (x_best, acc, loss_best, x_best_adv, arr_video if self.eps_increase_steps is not None else None)
        ...
        if not best_loss:
            ...
            for counter in range(self.n_restarts):
                ...
                if ind_to_fool.numel() != 0:
                    ...
                    if self.return_all_losses_confid:
                        ...
                        best_curr, acc_curr, loss_curr, adv_curr, video, all_losses, all_confid = res_curr
                    else:
                        best_curr, acc_curr, loss_curr, adv_curr, video = res_curr
            ...
            for counter in range(self.n_restarts):
                ...
                if self.return_all_losses_confid:
                    ...
                    best_curr, _, loss_curr, _, video = res_curr  # , first_run=(counter==0))
            ...
            if self.return_all_losses_confid:
                return adv_best, all_losses, all_confid, video
            else:
                return adv_best, video
