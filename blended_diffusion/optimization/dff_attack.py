...
def cone_projection(grad_temp_1, grad_temp_2, deg, subspace_projection=False):
    ...
    if subspace_projection:
        ...
        for i_image, grad_temp_1_image in enumerate(grad_temp_1):
            print(i_image)#, (grad_temp_1_image @ grad_temp_1_image.t()).shape, grad_temp_2[i_image].unsqueeze(0).t().shape)

            # cols of grad_temp_1_image are assumed to be basis vectors of a subspace for an image, on which we project grad_temp_2
            #grad_temp_1_image_transposed = grad_temp_1_image.t()
            #grad_temp.append((grad_temp_1_image_transposed @ (grad_temp_1_image @ grad_temp_1_image_transposed).inverse() @ grad_temp_1_image @ grad_temp_2[i_image].unsqueeze(0).t()).detach().cpu().squeeze())
        ...
        grad_temp = torch.stack(grad_temp, 0)#.t()
        ...
        #grad_temp_2 = _normalize_gradient(grad_temp_2)
        ...
        #print('radians are', angles_before.min(), angles_before.max(), angles_before)
...
class DiffusionAttack():
    def __init__(self, args) -> None:
        ...
        self.model.load_state_dict(
            torch.load(
                f"checkpoints/{self.args.diffusion_checkpoint}.pt",
                map_location="cpu"
            )
        )
...
#             pickle.dump(args, handle, protocol=pickle.HIGHEST_PROTOCOL)
        ...
        print('num_augmentations', self.args.aug_num)
        self.image_augmentations = ImageAugmentations(self.clip_size, self.args.aug_num, variance=self.args.variance)
    ...
    def _compute_probabilities(self, x, classifier, use_pca_components=False, y=None, current_bs=None, permuted_logits_order=None):
        if use_pca_components:
            ...
            target_class = self.args.class_id_spurious
            component_idx = self.args.component_idx_spurious

            # Temperature Wrapper (DataParallel (ResizeMean ) )
            last_layer = classifier.model.module.model.model.fc
            ###last_layer = classifier.model.module.model.fc

            act_space = ActivationSpace(classifier, last_layer, target_class, self.device)
            ...
            ###act_space.load(f'activation_spaces/timm,resnet50,nonrobust/{target_class}/act_space.npy')


            act = activations_with_grad(_map_img(x), classifier, last_layer, grad_enabled=True)
            ...
            #log_probs = log_probs[range(current_bs), y.view(-1)]
            #return pca_act[:, component_idx]*log_probs, probs
            if permuted_logits_order is not None:
                permuted_logits = torch.load('utils/'+permuted_logits_order)
                return pca_act[:, component_idx], probs[:, permuted_logits]
            else:
                return pca_act[:, component_idx], probs
    ...
    def edit_image_by_prompt(self, x, y, dir):
        ...
        def cond_fn_clean(x, t, y=None, eps=None):
            ...
            with torch.no_grad():
                ...
                if self.args.pca_component_lambda:
                    with torch.enable_grad():

                        #log_probs_1, probs_1 = self._compute_probabilities(self.image_augmentations(x_in), self.classifier, use_pca_components=True)
                        log_probs_1, probs_1 = self._compute_probabilities(x_in, self.classifier, use_pca_components=True)


                        target_log_confs_1 = log_probs_1
                        print('maximizing pca components', target_log_confs_1.shape, target_log_confs_1, probs_1[range(current_bs), y.view(-1)])

                        grad_class = torch.autograd.grad(target_log_confs_1.mean(), x,
                                                             retain_graph=self.args.classifier_lambda != 0)[0]

                    if self.args.enforce_same_norms:
                        grad_, norm_ = _renormalize_gradient(grad_class, eps)
                       
                        grad_class = self.args.pca_component_lambda * grad_

                    else:
                        grad_class *= self.args.pca_component_lambda
                ...
                if self.args.classifier_lambda != 0:
                    with torch.enable_grad():
                        #log_probs_1_order, probs_1_order = self._compute_probabilities(self.init_image, self.classifier)
                        ...
                        log_probs_1_subtracted = log_probs_1_order.detach().clone().cpu()
                        log_probs_1_subtracted[range(current_bs), y.view(-1)] = -float('inf')

                        if self.verbose:
                            print('log_probs_1_subtracted', log_probs_1_subtracted.shape, log_probs_1.shape)

                        time_start = time.time()
                        #for y_ in torch.cat((y.cpu().view(1, -1), log_probs_1_subtracted.sort(dim=1)[1][:, -9:].t()), 0):
                        for y_ in log_probs_1_subtracted.sort(dim=1)[1][:, -10:].t():
                            print('labels:', [self.imagenet_labels[yy_] for yy_ in y_.view(-1).tolist()])

                            assert all(y_.view(-1).cpu() != y.view(-1).cpu())

                            target_log_confs_1 = log_probs_1[range(current_bs), y_.view(-1)]
                            grad_1.append(torch.autograd.grad(target_log_confs_1.mean(), x, retain_graph=True)[0].detach().view(x.shape[0], -1))
                        print('grad_time', time.time() - time_start)
                        grad_1_final = []
                        time_start = time.time()
                        for ii in range(x.shape[0]):
                            grad_1_final.append(self._gram_schmidt(torch.stack([xx[ii] for xx in grad_1], 0).t()).t().cpu())
                        print('gram_schmidt_time', time.time() - time_start)
                        test_matrix = grad_1_final[0].cpu()
                        test_matrix = test_matrix.matmul(test_matrix.t())

                        # check that rows of the matrix form ONB
                        assert all(abs(test_matrix.diag() - 1) < 1e-4) #and abs(test_matrix.sum() - test_matrix.diag().sum()) < 1e-4
                        """
                        if self.verbose:
                            print('maximizing prob_log', target_log_confs_1.shape, target_log_confs_1, probs_1[range(current_bs), y.view(-1)])
                            ##print('maximizing prob_log', target_log_confs_1.shape, target_log_confs_1,
                            ##      probs_1[range(current_bs * self.args.aug_num), y.view(-1).repeat(self.args.aug_num)])

                        if self.args.second_classifier_type != -1:

                            if self.args.third_classifier_type != -1:
                                log_probs_3, probs_3 = self._compute_probabilities(x_in, self.third_classifier)
                                if self.verbose:
                                    print('third classifier probs_log', probs_3[range(current_bs), y.view(-1)])
                                target_log_confs_3 = log_probs_3[range(current_bs), y.view(-1)]
                                grad_3 = \
                                    torch.autograd.grad(target_log_confs_3.mean(), x,
                                                        retain_graph=True)[0]
                            ...
                            if int(self.args.second_classifier_type) in [20]:
                                ...
                                for i in range(int(self.args.aug_num)):
                                    ...
                                    log_probs_2, probs_2 = self._compute_probabilities(perturbed_images[i].unsqueeze(0), self.second_classifier,
                                                                                   permuted_logits_order=None) #'swapped_logits_v1_IN1000.pt')
                                ...
                                grad_2 = \
                                    torch.autograd.grad(target_log_confs_2.mean(), x,
                                                        retain_graph=keep_denoising_graph)[0]
                            #self.writer.add_images('augmented images',
                            #                       _map_img(self.image_augmentations(x_in)),
                            #                       self.tensorboard_counter
                            #                       )
                            ...
                            ##if self.verbose:
                            ##    #print('second classifier probs_log', (log_probs_1-log_probs_2).norm(p=2), probs_2[range(current_bs), y.view(-1)])
                            ##    print('second classifier probs_log', #(log_probs_1 - log_probs_2).norm(p=2),
                            ##          probs_2[range(current_bs * self.args.aug_num), y.view(-1).repeat(self.args.aug_num)])
                            #target_log_confs_2 = log_probs_2[range(current_bs), y.view(-1)]
                            ##target_log_confs_2 = log_probs_2[range(current_bs * self.args.aug_num), y.view(-1).repeat(self.args.aug_num)]
                            ...
                            #print('cone projection dist before', (grad_1 - grad_2).norm(p=2))
                            time_start = time.time()
                            #grad_class = cone_projection(grad_1_final, ##grad_1.view(x.shape[0], -1),
                            #                             grad_2.view(x.shape[0], -1).cpu(),
                            #                             self.args.deg_cone_projection,
                            #                             subspace_projection=True).view_as(grad_2).to(self.device)
                            ...
                            #print('cone projection dist after', (grad_1-grad_2).norm(p=2), (grad_class-grad_1).norm(p=2), (grad_class-grad_2).norm(p=2))
                            if self.args.third_classifier_type != -1:
                ...
                if self.args.lp_custom: # and self.args.range_t < self.tensorboard_counter:
                    if not keep_denoising_graph:
                        ...
                        with torch.enable_grad():
                            ...
                            #lp_dist = l1_loss(x_in, self.init_image)
                            #lp_grad = torch.autograd.grad(lp_dist, x)[0]
