...
from .utils import pca_component_loss
...
class APGDAttack():
    ...
    def attack_single_run(self, x, y, x_init=None, first_run=False, ODI_annealing_init=False):
        ...
        if not self.is_tf_model:
            ...
            elif self.loss.lower() == 'pca_component':
                criterion_indiv = lambda x, y: -pca_component_loss(x, self.model, reduction='none',
                                                                    args=self.args)
            else:
            ...
            for _ in range(self.eot_iter):
                if not self.is_tf_model:
                    with torch.enable_grad():
                        ...
                        if self.dist_regularizer is None:
                            loss_indiv = criterion_indiv(logits if self.loss != 'pca_component' else x_adv, y)
                        else:
        ...
        if self.return_all_losses_confid:
            ...
            if self.dist_regularizer is None:
                loss_indiv_ = criterion_indiv(logits_ if self.loss != 'pca_component' else x, y)
            else:
        ...
        for i in tqdm(range(self.n_iter)):
            ...
            if self.return_all_losses_confid:
                ...
                if self.dist_regularizer is None:
                    loss_indiv_ = criterion_indiv(logits_ if self.loss != 'pca_component' else x_adv, y)
                else:
                    loss_indiv_ = criterion_indiv(logits_, y, x, x_adv)
    ...
    def perturb(self, x, y=None, best_loss=False, x_init=None):
        ...
        assert self.loss in ['ce', 'dlr', 'ce-targeted-cfts', 'ce-targeted-cfts-conf', 'dlr-targeted',
                             'ce-targeted-cfts-conf-binary', 'pca_component']
...
class APGDAttack_targeted(APGDAttack):
    ...
    def perturb(self, x, y=None, best_loss=False, x_init=None):
        ...
        assert self.loss in ['dlr-targeted', 'margin-targeted', 'ce-targeted-cfts-conf-binary', 'pca_component']  # 'ce-targeted'
        if not y is None and len(y.shape) == 0:
            x.unsqueeze_(0)
            y.unsqueeze_(0)
        self.init_hyperparam(x)
