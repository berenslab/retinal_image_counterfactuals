...
from .utils import logits_diff_loss, conf_diff_loss, confidence_loss, reduce, log_confidence_loss, pca_component_loss
    ...
    def _get_loss_f(self, x, y, targeted, reduction):
        ...
        if isinstance(self.loss, str):
            if self.loss.lower() in ['crossentropy', 'ce']:
                ...
                if not targeted:
                    l_f = lambda data, data_out: pca_component_loss(data, self.classifier, reduction=reduction, args=self.args)
                else:
                    l_f = lambda data, data_out: -pca_component_loss(data, self.classifier, reduction=reduction, args=self.args)
